...
import { Plus, Trash2, X, Search, Activity, ChevronRight, ChevronLeft } from 'lucide-react';
import { toast } from 'sonner';
import type { TypeProgramme, JourProgramme, Participant, Exercice } from '../../types';
...
// (ModelesProgrammePage.tsx, sans bénéficiaire). La prop estModele n'ajuste
// que les libellés — la structure de données (WizardData) et la logique du
// formulaire sont strictement identiques dans les deux cas.

// ── Types wizard ─────────────────────────────────────────────────────────────

export interface FormExercice {
  tempId: string;
  nom: string;
  categorie: string;
  description: string;
...
export function newExercice(): FormExercice {
  return {
    tempId: uuidv4(),
    nom: '',
    categorie: 'Équilibre',
...
export function newSeance(label: string): FormSeance {
...
export const JOUR_COURT: Record<JourProgramme, string> = {
...
export const NIVEAU_STYLE: Record<'1' | '2' | '3', { bg: string; color: string; label: string; emoji: string }> = {
...
// ── Sélecteur d'exercice (bibliothèque + création manuelle) ───────────────────
...
function Step1({ data, onChange, estModele }: {
  data: WizardData;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      <div>
        <label style={labelStyle}>{estModele ? 'Nom du modèle *' : 'Nom du programme *'}</label>
        <input
          value={data.nom}
          onChange={e => onChange({ nom: e.target.value })}
          placeholder={estModele ? 'ex: Rééducation post-chute' : 'ex: Programme Équilibre'}
          style={inputStyle}
        />
      </div>
      <div>
      ...
      <div>
        ...
        <div style={{ fontSize: 11, color: '#94A3B8', marginTop: 4 }}>
          {estModele
            ? 'Nombre de séances que le bénéficiaire devra réaliser seul, une fois ce modèle appliqué.'
    ...
    <div>
      ...
      {pickerFor !== null && (
        <ExercicePickerModal
          ...
          onAddManual={async (ex, toLib) => {
            if (toLib) {
              const created = await saveExercicePersonnalise({
                nom: ex.nom,
              ...
              if (created) {
                ...
                toast.error('Erreur lors de la sauvegarde dans la bibliothèque');
              }
            }
...
export interface ProgrammeWizardModalProps {
  step: number;
...
export function ProgrammeWizardModal({
  ...
  function handleNext() {
    ...
    if (step === 2 && data.seances.some(s => !s.nom.trim())) {
  ...
  const titre = estModele
    ? (isEditing ? 'Modifier le modèle' : 'Nouveau modèle')
    : (isEditing ? 'Modifier le programme' : 'Nouveau programme');

  const labelSauvegarder = estModele
    ? (isEditing ? '✅ Enregistrer les modifications' : '✅ Enregistrer le modèle')
    : (isEditing ? '✅ Enregistrer les modifications' : '✅ Sauvegarder et partager');
        ...
        <div style={{ background: 'var(--color-ink)', padding: '20px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div style={{ fontSize: 16, fontWeight: 800, color: 'white' }}>
              {titre} — Étape {step}/{STEP_LABELS.length}
            </div>
        ...
        <div style={{
          ...
          {step < 4 ? (
            ...
            <button onClick={onSave} disabled={saving} style={{ ...btnPrimary, opacity: saving ? 0.7 : 1 }}>
              {saving ? 'Sauvegarde…' : labelSauvegarder}
            </button>
          )}
        </div>
      </div>
