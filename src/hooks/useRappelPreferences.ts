...
function rowToPrefs(row: Record<string, unknown>): RappelPreferences {
  return {
    ...
    relanceExercicesActif: Boolean(row.relance_exercices_actif),
    relanceExercicesSeuilJours: Number(row.relance_exercices_seuil_jours),
  };
}
...
export function useRappelPreferences(participantId?: string) {
  ...
  async function enregistrer(next: RappelPreferences): Promise<boolean> {
    ...
    const payload = {
      ...
      relance_exercices_actif: next.relanceExercicesActif,
      relance_exercices_seuil_jours: next.relanceExercicesSeuilJours,
    };
