...
export function dbToParticipant(row: any): Participant {
  return {
    ...
    codePostalNaissance: row.code_postal_naissance ?? undefined,
    ...
    coordonnees: row.coordonnees_lat != null ? {
      ...
      geocodeeAt: row.coordonnees_geocodee_at,
      adresseNormalisee: row.coordonnees_adresse_normalisee,
    } : undefined,
    geocodeFailed: row.geocode_failed ?? false,
    disponibilites: row.disponibilites ?? undefined,
    token: row.token,
    profilHandicap: row.profil_handicap ?? undefined,
    modeDeplacementHabituel: row.mode_deplacement_habituel ?? undefined,
...
export function participantToDb(p: Omit<Participant, 'bilans' | 'programmes'>): Record<string, unknown> {
  return {
    ...
    coordonnees_geocodee_at: p.coordonnees?.geocodeeAt ?? null,
    coordonnees_adresse_normalisee: p.coordonnees?.adresseNormalisee ?? null,
    geocode_failed: p.geocodeFailed ?? false,
    disponibilites: p.disponibilites ?? null,
    token: p.token,
    profil_handicap: p.profilHandicap ?? null,
    mode_deplacement_habituel: p.modeDeplacementHabituel ?? null,
