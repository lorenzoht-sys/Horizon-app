...
export function participantToDb(p: Omit<Participant, 'bilans' | 'programmes'>): Record<string, unknown> {
  return {
    ...
    poids: p.poids ?? null,
    nom_naissance: p.nomNaissance ?? null,
...
export function dbToBilan(row: any): Bilan {
  return {
    ...
    tm6: {
      mode: row.tm6_mode ?? undefined,
      distanceMetres: row.tm6_distance_metres,
      repetitions: row.tm6_repetitions ?? null,
      fcAvant: row.tm6_fc_avant,
      fcApres: row.tm6_fc_apres,
      fc1min: row.tm6_fc_1min ?? null,
      fc2min: row.tm6_fc_2min,
      spo2Avant: row.tm6_spo2_avant,
      spo2Apres: row.tm6_spo2_apres,
      spo21min: row.tm6_spo2_1min ?? null,
      spo22min: row.tm6_spo2_2min,
      borgRPE: row.tm6_borg_rpe,
      dureeMode: row.tm6_duree_mode ?? null,
      dureeCibleSecondes: row.tm6_duree_cible_secondes ?? null,
      dureeReelleSecondes: row.tm6_duree_reelle_secondes ?? 360,
      nbPauses: row.tm6_nb_pauses ?? 0,
      dureePausesSecondes: row.tm6_duree_pauses_secondes ?? 0,
      notesPauses: row.tm6_notes_pauses ?? '',
      pausesDetail: row.tm6_pauses_detail ?? undefined,
    ...
    marche10m: (row.marche10m_habituel != null || row.marche10m_max != null)
      ? { habituel: row.marche10m_habituel ?? null, max: row.marche10m_max ?? null }
      : null,
    adl: row.adl_data ?? null,
    iadl: row.iadl_data ?? null,
    douleurEva: row.douleur_eva ?? null,
...
export function bilanToDb(participantId: string, b: Omit<Bilan, 'id'> & { id?: string }): Record<string, unknown> {
  return {
    ...
    tm6_spo2_1min: b.tm6?.spo21min ?? null,
    tm6_spo2_2min: b.tm6?.spo22min ?? null,
    tm6_borg_rpe: b.tm6?.borgRPE ?? null,
    tm6_duree_mode: b.tm6?.dureeMode ?? null,
    tm6_duree_cible_secondes: b.tm6?.dureeCibleSecondes ?? null,
    tm6_duree_reelle_secondes: b.tm6?.dureeReelleSecondes ?? null,
    tm6_nb_pauses: b.tm6?.nbPauses ?? null,
    tm6_duree_pauses_secondes: b.tm6?.dureePausesSecondes ?? null,
    ...
    marche10m_max: b.marche10m?.max ?? null,
    adl_data: b.adl ?? null,
    iadl_data: b.iadl ?? null,
    douleur_eva: b.douleurEva ?? null,
