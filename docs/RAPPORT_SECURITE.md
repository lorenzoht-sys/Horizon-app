...
- **Statut** : **Corrigé (code réécrit le 2026-08-19, non testé sur staging/prod).** Version initiale invalidée par [RÉG-01] (retirait toute écriture `authenticated`, cassait `useTm6Variantes.ts`) — réécrite : `tm6_variantes` a désormais une colonne `praticien_id` (nullable, NULL = entrée système), policies d'écriture scopées au propriétaire (`praticien_id = auth.uid()`) au lieu d'un verrou total service_role, trigger d'auto-remplissage (même fonction que le reste du projet), et `REVOKE TRUNCATE` explicite pour `authenticated`. `useTm6Variantes.ts` continue de fonctionner (une nouvelle variante créée par un praticien lui appartient). Détail complet dans [RÉG-01] ci-dessous, qui documente aussi la réécriture. Pas de faille active sur donnée patient (constat inchangé). **Non appliqué nulle part** (ni staging ni prod) — staging ne reproduit de toute façon pas l'état vulnérable de prod actuellement (voir `docs/ETAT_AUDIT.md`), donc pas encore testable automatiquement.

  **⚠️ Complément 2026-08-19 — GRANTs réels de `tm6_variantes` en prod, vérifiés par ACL brute** (`aclexplode(pg_class.relacl)`, contourne la restriction de visibilité de `information_schema.role_table_grants` qui avait masqué ce qui suit lors d'une première vérification) :
  ```
  authenticated : DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE
  service_role  : DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE
  anon          : aucune ligne
  ```
  `authenticated` a un `GRANT ALL` complet, **y compris `TRUNCATE`**. C'est un point structurel important : **la RLS est la SEULE barrière côté `authenticated` sur cette table** — les GRANTs eux-mêmes sont aussi larges que possible. Or **`TRUNCATE` n'est jamais soumis aux policies RLS** en PostgreSQL (RLS ne filtre que SELECT/INSERT/UPDATE/DELETE ligne par ligne ; TRUNCATE vide la table entière sans passer par les policies). Concrètement : tant que `authenticated` garde le GRANT TRUNCATE, aucune policy RLS — même parfaitement écrite — ne peut empêcher un praticien authentifié de vider `tm6_variantes` en une seule commande. **Nuance sur l'exploitabilité réelle** : PostgREST (l'API que l'app et un attaquant utilisant la clé publique consomment) ne mappe TRUNCATE sur aucune route REST standard — ce n'est donc pas exploitable via l'API HTTP normale de l'app aujourd'hui (confirmé : aucune fonction RPC dans ce projet n'appelle TRUNCATE). Le risque est réel uniquement pour quelqu'un disposant d'un accès Postgres direct authentifié en tant que `authenticated` (scénario différent de "n'importe qui avec la clé anon"), donc la gravité pratique reste modérée, mais **le futur correctif RÉG-01 doit inclure un `REVOKE TRUNCATE` explicite**, pas seulement resserrer les policies — sinon il laisse cette porte ouverte en pensant l'avoir fermée. **Non vérifié si ce modèle (GRANT ALL large + RLS comme seule barrière, y compris TRUNCATE) est le défaut sur les autres tables du projet** — je n'ai vérifié que `tm6_variantes`. Requête pour vérifier l'ensemble du schéma `public` d'un coup (à lancer sur prod, lecture seule) :
  ```sql
...
- **Correctif appliqué (réécriture, 2026-08-19)** : option (b) retenue — `tm6_variantes` gagne une colonne `praticien_id` (nullable, `ON DELETE SET NULL`), un trigger `BEFORE INSERT` (`set_praticien_id_from_auth()`, même fonction que le reste du projet) qui auto-remplit `praticien_id` depuis `auth.uid()` si absent, et des policies `authenticated` scopées : SELECT reste ouvert à tous (catalogue partagé, lecture non sensible), INSERT/UPDATE/DELETE exigent `praticien_id = auth.uid()`. Une entrée système (`praticien_id IS NULL`) n'est donc jamais modifiable/supprimable par `authenticated` (`NULL = auth.uid()` n'est jamais vrai en SQL — protection par construction, pas par cas particulier). `REVOKE TRUNCATE ON tm6_variantes FROM authenticated` ajouté, comme identifié dans le complément ACL du 2026-08-19 (RLS ne filtre jamais TRUNCATE). `useTm6Variantes.ts` (`select`/`insert`/`delete` via client authentifié direct) continue de fonctionner : une variante créée par un praticien lui appartient et lui reste gérable. Note : `supprimer()` du hook n'est appelé nulle part dans `src/` au 2026-08-19 (grep vérifié) — la policy DELETE est écrite par cohérence, pas parce que c'est utilisé aujourd'hui.
- **Statut** : Résolu côté code, **non appliqué et non testé** (ni staging ni prod — staging ne reproduit pas l'état vulnérable de prod actuellement, voir `docs/ETAT_AUDIT.md`). L'avertissement "NE PAS APPLIQUER EN L'ÉTAT" a été retiré de `20260817_securite_01_tm6_variantes_rls.sql` (la nouvelle version ne casse plus `useTm6Variantes.ts`), mais rien ne remplace un test réel avant un passage en prod. Le test `[F-01]` du harnais reste valable tel quel (il vérifie "praticien B ne peut pas modifier/supprimer une ligne qui ne lui appartient pas", ce qui reste la bonne assertion avec la nouvelle policy).
...
### [F-13] IDOR sur `api/patient/retour-seance.ts` — `seanceId` du body inséré sans vérifier son appartenance au participant
- **Gravité** : Critique — écriture possible sur une ressource (`retours_seance.seance_id`) rattachée à un autre patient, par un patient authentifié quelconque.
- **Surface** : API
- **Preuve** : `api/patient/retour-seance.ts` (avant correctif) — `seanceId` extrait du body (L27), validé seulement comme "string ou absent" (L35-37), puis inséré tel quel dans `retours_seance.seance_id` (L68), **sans jamais vérifier qu'il référence une ligne `seances_patient` appartenant à `participantId`** (le participant authentifié, dérivé du JWT — ça, c'était bien vérifié). La route utilise `service_role` (contourne RLS), donc rien en base n'empêchait ça non plus.
- **Impact concret** : un patient authentifié (code d'accès valide, donc "juste" un patient normal, pas besoin d'être sophistiqué) peut soumettre un `seanceId` appartenant à un autre patient — son retour subjectif (Borg RPE, bien-être) s'associe alors à la séance d'un tiers. Pas une fuite de lecture (la route n'a jamais renvoyé la ligne insérée ni permis de la relire), mais une pollution d'intégrité de donnée de santé : un praticien consultant l'historique de séance d'un patient pourrait voir un retour qui n'est pas le sien, sans aucune indication que la donnée est corrompue.
