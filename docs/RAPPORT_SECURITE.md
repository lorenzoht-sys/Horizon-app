...
- **Correctif à revoir** (pas encore écrit) : soit (a) créer une route `api/tm6-variantes/*` en `service_role` et faire pointer `useTm6Variantes.ts` dessus avant de verrouiller la table, soit (b) garder une policy `authenticated` restreinte pour INSERT/DELETE (ex. limitée à un rôle praticien vérifié) plutôt que de tout réserver à `service_role`. Dans les deux cas, **inclure un `REVOKE TRUNCATE ON tm6_variantes FROM authenticated`** — les GRANTs prod donnent TRUNCATE à `authenticated`, un privilège que RLS ne filtre jamais (voir complément 2026-08-19 dans le statut F-01 ci-dessus) ; resserrer uniquement les policies laisserait ce chemin ouvert. Décision à prendre avant de réécrire la migration — hors scope de cette étape (audit du harnais, pas correction).
